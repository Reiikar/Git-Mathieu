...
To be concrete, let $\Sigma \su \Aut(L/\Fqm)$ be a finite subgroup, 
%denote by $\mathrm{Orb}_{\Sigma}(Q)$ the orbit of $Q \in \PP_L$ under its action.
...
\begin{def1} Given an $\ag$-code $\mathcal{C} = C_{\calL}(\Y,\QR,G)$ on a smooth irreducible projective curve $\Y$ with function field $L$ over $\Fqm$, such that $\QR$ and $G$ are invariant under the action of $\Sigma \su \Aut(L/\Fqm)$, we define its invariant code as the subcode 
...
In what follows, we study the structure of the invariant code, which will be an important result for the next parts. In particular, we show that this subcode is also an $\ag$-code, defined on the quotient curve $\Y/\Sigma$, whose function field is precisely the fixed field $K:=L^{\Sigma}$ of $L$. Let us start with the following lemma.

\begin{lem1} \label{inv}
Let $\QR$ and $G$ be as in Definition~$\ref{defagcode}$, and suppose that they are invariant under an automorphism $\sigma \in \Sigma \su \Aut(L/\Fqm)$. If $c = \operatorname{Ev}_{\QR}(g) \in C_{\calL}(\Y,\QR,G)$ is such that $\sigma(c) = c$, then $g$ is $\sigma$-invariant, i.e. $g\circ \sigma = g$. 
\end{lem1}
...
Consider an extension $L/K$ of function fields corresponding to a separable morphism of curves $\pi:\cal{Y}\rightarrow\cal{X}$.
Given a divisor $G \in \Div(L)$, we denote by $\tilde{G} \in \Div(K)$ the largest divisor (according to the degree) such that
\[\pi^*\tilde{G} \leq G.\]
\end{def1} 
...
Let $L/K$ be a Galois extension of function fields over $\Fqm$, with Galois group $\Sigma$,
...
\begin{proof}
\begin{itemize}
\item[$(i)$] First observe that $\calL_L(G)^{\Sigma}=\calL_L(G)\cap L^\Sigma=\calL_L(G)\cap K$.
Now given $h\in K$ we have $(h)_L=\pi^*((h)_K)$, and thus we have $h\in\calL_L(G)$ iff $\pi^*((h)_K)\geq-G$.
But by Definition~\ref{pousseavant} this means precisely $(h)_K\geq-\tilde{G}$, \emph{i.e.} $h\in\calL_K(\tilde{G})$.
\item[$(ii)$] Given $R\in\PP_K$ and $S\in\PP_L$ with $S|R$, we have
$$\pi^*R=e(S|R)\sum\limits_{Q \in \mathrm{Orb}_{\sigma}(S)} Q.$$
As a consequence, for integers $x$ and $t$, we have $$\pi^*(xR)\leq t\sum\limits_{Q \in \mathrm{Orb}_{\sigma}(S)} Q$$ iff $$x\leq\left\lfloor\dfrac{t}{e(S|R)}\right\rfloor.$$ The conclusion follows by linearity of $\pi^*$.
\end{itemize}
\end{proof}
%\color{black}

\begin{thm} [Structure of the invariant code] \label{invstruct}
Let $\mathcal{C} := C_{\calL}(\Y,\QR,G)$ be an $\mathrm{AG}$-code defined on a curve $\Y$ with function field $L$, invariant under the action of $\Sigma \su \Aut(L/\Fqm)$. Then its invariant code is also an $\mathrm{AG}$-code, defined on the quotient curve $\Y/\Sigma$. In particular, there exist a support $\PR$ and a divisor $\tilde{G}$ on $\Y/\Sigma$ such that
...
In order to recover an equation of $\Y$, we need to find the minimal polynomial $H$ of $y$ over $K$. Denote by $\ell=[L:K]$ the degree of the extension $L/K$ (\textit{i.e.} the order of its Galois group) and suppose that we are given a set of $r$ places of degree one in $K$, say $\PR = \{P_1,...,P_r\}$, that totally split in $L/K$. For any $1 \leq i \leq r$, we then have
...
Let us now explain what we plan to do. The main idea is first to recover the finite set of points $\QR$ in $\Y$, in order to recover the minimal polynomial of $y$ using interpolation. Thanks to hypothesis $2$, we know the coordinates of the rational points corresponding to the places $P_i$'s in $K$. Since places in $\QR$ are extensions of places in $\PR$ (\textit{i.e.} for all $Q \in \mathcal{Q}$, there exist $P \in \PR$ such that $P=Q\cap K$), we only need to recover the $y$-evaluation of points in $\QR$. Thus, we are led to find the row vector
\begin{equation}
...
\item The quotient curve $\X$ is known (and thus both polynomials $F_1$ and $F_2$), as well as the structure of the invariant code of $\mathcal{C}$, \textit{i.e.} $\PR$ and $\tilde{G}$ such that $\mathcal{C}^{\sigma} = \ssag_q(\X,\PR,\tilde{G})$;
