...
\begin{itemize}
\item the divisor of poles $(h_k)_\infty^K$ is known, and of degree $\deg(h_k)_\infty^K<n$; or:
\item the divisor of poles $(h_k)_\infty^K$ is unknown, but at least it is known that it has degree $\deg(h_k)_\infty^K<n/2.$
\end{itemize}
Then the polynomial $H(T)$ is entirely determined by the evaluation vector $\mathbf{y} = (y(Q_{i,j}))_{i,j}$.
\end{prop1}
\begin{proof}
By Galois theory, the roots of $H$ are $\sigma_1(y),\dots,\sigma_\ell(y)$, where we write $\Sigma=\{\sigma_1,\dots,\sigma_\ell\}$; and so for each $k$ we have
$$h_k=(-1)^{\ell-k}s_{\ell-k}(\sigma_1(y),\dots,\sigma_\ell(y))$$
...
\end{equation}
We have to show that \eqref{systemeinterpolation} entirely determines $h_k$.

In the case that $D_k=(h_k)_\infty^K$ is known, and of degree $\deg(D_k)<n$, let $h_k'\in\mathcal{L}(D_k)$ be another solution of \eqref{systemeinterpolation}. Then we have $h_k-h_k'\in\mathcal{L}(D_k)$ but $h_k-h_k'$ has at least $n$ zeroes, hence $h_k-h_k'=0$.
