...
\item We have full knowledge of the invariant code $\C^{\Sigma}$: more precisely, we know the quotient curve $\X$ (and thus its function field $K$), the finite set of places $\PR$ of $K$ and the divisor $\tilde{G} \in \Div(K)$.
\item We know the action of $\Sigma$ on $\mathcal{C}$. In particular, for any known place $P \in \PR$, we know the positions of its $\ell$ extensions in the ordered set $\QR$, and we know how they are permuted by $\Sigma$.
