...
There is an anti-equivalence of categories between smooth, irreducible, projective curves over $\F$, with non-constant morphisms of curves over $\F$, and function fields over $\F$, with field morphisms over $\F$, which to a curve $X$ associates its function field $K=\F(X)$.
\end{thm}

\noindent Throughout this paper, the word ``curve'' will be a shorthand for ``smooth, irreducible, projective curve''. Moreover, in accordance with \cite[sec.~1.4~ff.]{Sti}, when speaking about a function field $K$ over $\F$, we will always implicitly assume that the field of constants of $K$ is reduced to $\F$. Alternatively, that means that we will always implcitly assume that our curves are \emph{geometrically irreducible}.

\vspace*{0.2cm}

\noindent A function field $K$ over $\F$ can always be described as $K=\F(x)[y]$ where $x$ is transcendental over $\F$ and $y$ is a primitive element for $K$ over $\F(x)$.
...
Given a tower $\F(x) \su K \su L$ of function fields over $\F$, let us consider a place  $P \in \PP_K$ and one of its extension $Q \in \PP_L$ (noted by $Q|P$). The ramification index is as usual denoted by $e(Q|P)$. \\
