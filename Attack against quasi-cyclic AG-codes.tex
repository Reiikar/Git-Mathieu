...
% \s
%\begin{expl} 
%If $\Sigma = \langle\sigma\rangle$ is cyclic of order $\ell$ generated by $\sigma \in \Aut(L)$ and if $\QR$ and $G$ are $\sigma$-invariant, the code $C_{\calL}(\Y,\QR,G)$ is said to be $\ell$-quasi-cyclic. In this case, the code $\ssag_q(\Y,\QR,G)$ is also $\ell$-quasi-cyclic.
%\end{expl}
...
\[c = (f(Q_{i,j}))_{i,j}\]
%\color{blue}(keep in mind that $c \in \fq^n$ since we are looking at SSAG) \color{black}\\
...
%Moreover, this choice of $D$ is optimal (ie. the biggest, see. Definition~\ref{pousseavant}) since $d/\ell$ can not be an integer (recall that $d$ and $\ell$ are coprime).
\end{proof}
