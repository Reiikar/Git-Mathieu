...
\[L(A) = \{z \in K \ | \ (z)^L \geq -A\} \cup \{0\},\]
of dimension $l(A):= \dim_{\fq}(L(A))$.
...
Let $\Y$ be a smooth projective curve over $\fqm$ with function field $L = \fqm(\Y)$, $\QR = \{Q_1,...,Q_n\}$ be a set of $n$ distinct places of degree one in $L$  and $G \in \Div(L)$ be a divisor such that $\Supp(G) \cap \QR = \emptyset$. Suppose that $\deg(G)<n$. Then the AG-code associated to the triple $(\Y,\QR,G)$ is defined  as the $\fqm$-vector space
...
\color{purple} Préciser la forme de $G'$ ? \color{black}
\end{prop1}
...
\begin{def1} Given an $\ag$-code $\mathcal{C} = C_L(\Y,\QR,G)$ on a curve $\Y$ with function field $L$, such that $\QR$ and $G$ are invariant under the action of $\Sigma \su \Aut(L)$, we define its invariant code as the subcode 
...
\begin{rq1} \label{rqq}
The above theorem can be made a bit more precise, since we can make $\PR$ and $\tilde{G}$ explicit. Indeed, the divisor $\tilde{G}$ is nothing but the one introduced  Proposition~\ref{invrr} $(ii)$, and \color{purple} \[\PR := \\ \{P \in \PP_{L^{\Sigma}} \ ; \ Q|P\} = \{Q \cap L^{\Sigma} \ , Q \in \QR\} \] \color{black}In particular, they can be described by using the ramification in the cover of curves $\Y \longmapsto \Y/\Sigma$.
\end{rq1}
...
\vspace*{0.1cm}
\textbf{\underline{Algorithm 1 : Security reduction in Kummer case}}

\s

\textbf{\underline{Inputs:}} 
\begin{enumerate}
\item[-] A parity check matrix $\mathbf{H}_{\mathrm{pub}}$ of the public code;
...
\textbf{\underline{Outputs:}}
\begin{enumerate}
\item[-] The polynomial $f$;
\item[-] The secret structure $(\PR,G)$.
\end{enumerate}

\begin{enumerate}
\item[1.] $x \longleftarrow (\underbrace{x(P_1),...,x(P_1)}_{\ell\mathrm{-fois}},...,x(P_r),...,x(P_r))$
\item[2.] $D \longleftarrow \tilde{G} - \lceil d/{\ell} \rceil \cd P_{\infty} \in \Div(\PP^1)$
\item[3.] $s\longleftarrow \dim(D)$
\item[4.] $M \longleftarrow$ set of primitive $\ell$-th roots of unity
\item[5.] cpt $:= 0$
\item[6.] while cpt $:=0$ do 
\item[7.] $\quad \quad \xi \xleftarrow[]{\$} M $
\item[8.] $\quad \quad \mathrm{Exclude}(M,\xi)$
\item[9.] $\quad \quad S \longleftarrow \mathrm{Solve}(\Delta(\xi))$
\item[10.] $\quad \quad$ if $\dim(S)$ $=1$ then
\item[11.] $\quad \quad \quad \quad$ cpt $:=1$
\item[12.] $\quad \quad \quad \quad$ $y^* \xleftarrow[]{\$} S \backslash \{0\}$
\item[13.] $\quad \quad \quad \quad$ $f \longleftarrow$ $\mathrm{Interpolate}(x,y^*)$
...
\vspace*{0.1cm}
\textbf{\underline{Algorithm 2 : Brute force on $\ssag$}}

\s

\textbf{\underline{Inputs:}}
\begin{enumerate}
\item[-] A generator matrix $\mathbf{G}$ of the invariant $\ssag$-code $\mathcal{C}$;
...
\begin{enumerate}
\item[1.] $S \longleftarrow \{P \in \calH \ | \ \deg(P)=1\}$ \quad // the lenght is fixed
\item[2.] for $[G] \in \Cl^r(\calH)$ do
\item[3.] $\quad \mathcal{C}' \longleftarrow \ssag_q(\calH,S,G)$
\item[4.] $\quad$ for $[\mathbf{w}] \in \fqm^n/\fq^n$ do  $\quad$ // $\mathbf{w}$ representative of [$\mathbf{w}$]
\item[5.] $\quad \quad $ for $\mathcal{I} \su \{1,...,N(\calH)-1\}$ with $|\mathcal{I}|=n$ do
\item[6.] $\quad \quad \quad$ $\mathcal{C}'_{\mathcal{I}} \longleftarrow  \mathrm{Punct}_{\mathcal{I}}(\mathcal{C}')$ 
\item[7.] $\quad \quad \quad \pi \longleftarrow \mathrm{SSA}(\mathbf{w} \star \mathcal{C}'_{\mathcal{I}},\mathcal{C})$ $\quad$ // SSA return a permutation or '?'
\item[8.] $\quad \quad \quad$ If $\pi \in S_n$ then
\item[9.] $\quad \quad \quad \quad S_{\mathcal{I}} \longleftarrow \{P_i \in S \ | \ i \in \mathcal{I}\}$
\item[10.] $\quad \quad \quad \quad$ return $\pi(S_{\mathcal{I}}),G$ and $\mathbf{w}$ 
\end{enumerate}
\end{minipage}
}
\end{center}
