...
\Input{A parity check matrix $\mathbf{H}_{\mathrm{pub}}$ of the public code, the invariant support $\PR=\{P_1,\cdots,P_r\}$ and divisor $\tilde{G}$. Also we are given the quasi-cyclicity order $\ell > 0$ and the integer $d:=\deg(f)$.} 
\Output{The polynomial $f$ and the secret structure $(\QR,G)$.}
{
...
\While{$\mathrm{cpt} = 0$}{$\xi \xleftarrow[]{\$} M$ \\
	...
	\If{$\dim(S)=1$}{cpt := 1 \\
		...
		$\QR \longleftarrow \{Q_{ij}=(x_{ij}:y_{ij}:1)\}$
	} 
} 
$G \longleftarrow \pi^*(\tilde{G})$ \\
\Return{$f$, $\QR \ \mathrm{and} \ G$}
}
\end{algorithm}
